            ...
            callback: function(res) {
                if (res.message) {
                    ...
                    (committee.table_zxij || []).forEach(row => {
                        ...
                        // Fetch defaults first, then override rate/qty
                        frappe.model.set_value(item_row.doctype, item_row.name, "item_code", row.item).then(() => {
                            frappe.model.set_value(item_row.doctype, item_row.name, "description", row.description);
                            frappe.model.set_value(item_row.doctype, item_row.name, "qty", row.quantity);
                            frappe.model.set_value(item_row.doctype, item_row.name, "rate", row.rate);
                            frappe.model.set_value(item_row.doctype, item_row.name, "uom", row.uom);
                            frappe.model.set_value(item_row.doctype, item_row.name, "schedule_date", row.required_date);
                            frappe.model.set_value(item_row.doctype, item_row.name, "fg_item", row.finished_good);
                            frappe.model.set_value(item_row.doctype, item_row.name, "custom_drawing_no", row.drawing_no);
                            frappe.model.set_value(item_row.doctype, item_row.name, "fg_item_qty", row.finished_good_quantity);

                            if (committee.project) {
                                frappe.model.set_value(item_row.doctype, item_row.name, "project", committee.project);
                            }
                        });
                    });
            ...
            callback: function(res) {
                if (res.message) {
                    ...
                    (approval.table_higc || []).forEach(row => {
                        ...
                        frappe.model.set_value(item_row.doctype, item_row.name, "item_code", row.item).then(() => {
                            frappe.model.set_value(item_row.doctype, item_row.name, "description", row.description);
                            frappe.model.set_value(item_row.doctype, item_row.name, "qty", row.qty);
                            frappe.model.set_value(item_row.doctype, item_row.name, "rate", row.rate);
                            frappe.model.set_value(item_row.doctype, item_row.name, "uom", row.uom);
                            frappe.model.set_value(item_row.doctype, item_row.name, "schedule_date", row.required_date);
                            frappe.model.set_value(item_row.doctype, item_row.name, "fg_item", row.finished_good);
                            ...
                            if (approval.project) {
                                frappe.model.set_value(item_row.doctype, item_row.name, "project", approval.project);
                            }
                        });
                    });
